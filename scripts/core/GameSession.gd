...
const PASS_COOLDOWN_TURNS: int = 2  # 虚手冷却：自上次虚手后需 2 个己方回合才能再次虚手
var pass_cooldown: Dictionary = {}  # color -> 自上次虚手后的己方回合计数（达到 PASS_COOLDOWN_TURNS 即可虚手）
var skip_pass_limits: bool = false  # true 时跳过虚手冷却限制（回放/审计旧棋谱用，旧规则无此限制）
...
func _reset_state() -> void:
	...
	pass_cooldown = { Const.BLACK: PASS_COOLDOWN_TURNS, Const.WHITE: PASS_COOLDOWN_TURNS }
	game_over = false
...
func do_pass(color: int) -> Dictionary:
	...
	if color != to_move:
		...
		outcome.reason = "非该方行棋"
		return outcome
	# 虚手冷却：自上次虚手后需 PASS_COOLDOWN_TURNS 个己方回合才能再次虚手
	# （回放/审计旧棋谱时通过 skip_pass_limits 绕过，旧规则无此限制）
	if not skip_pass_limits and pass_cooldown.get(color, PASS_COOLDOWN_TURNS) < PASS_COOLDOWN_TURNS:
		outcome.ok = false
		var remaining: int = PASS_COOLDOWN_TURNS - int(pass_cooldown.get(color, 0))
		outcome.reason = "虚手冷却中（还需 %d 个己方回合）" % remaining
		return outcome
	# 校验通过，取行棋前快照（终局虚手不 push，因 _commit_turn 不被调用）
	_begin_undo_snapshot()
	consecutive_passes += 1
	pass_cooldown[color] = 0  # 虚手后重置该方己方回合计数
	outcome.passed = true
...
func _commit_turn(outcome: Dictionary, color: int, did_place: bool) -> void:
	# 任何实际行棋（落子/部署/弹子）取消连续虚手；行棋方累计己方回合计数（用于虚手冷却）
	if not outcome.passed:
		consecutive_passes = 0
		pass_cooldown[color] = min(int(pass_cooldown.get(color, 0)) + 1, PASS_COOLDOWN_TURNS)
	ply += 1
	outcome.ply = ply
...
func _take_snapshot() -> Dictionary:
	return {
		...
		"pass_cooldown": pass_cooldown.duplicate(true),
		"game_over": game_over,
...
func undo() -> Dictionary:
	...
	pass_cooldown = snap.pass_cooldown
...
func clone() -> GameSession:
	...
	s.pass_cooldown = pass_cooldown.duplicate(true)
