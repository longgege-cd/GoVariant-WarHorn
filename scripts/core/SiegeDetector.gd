...
static func _is_surrounded_by_opponent(board: BoardModel, group: Dictionary) -> bool:
	...
	# 从棋盘边缘出发flooding（可穿过空点+己方棋子，仅被对方棋子阻挡）
	var outside: Dictionary = {}
	var flood_stack: Array = []
	for c in range(size):
		for r in [0, size - 1]:
			var idx: int = r * size + c
			if board.get_at(r, c) != opp and not outside.has(idx):
...
# 真眼判定：正交邻居全为该组棋子(或边界) + 对角线控制
# 对角线控制：非己方（对方棋子或空点）数量判定
#   - 边/角：所有棋盘内对角线必须是己方棋子（非己方=0）
#   - 中心：最多 1 个非己方对角线（对方或空）
# 注意：空点算"非己方"，因为对方可能下在那里破坏眼
static func _is_true_eye(board: BoardModel, row: int, col: int, group_set: Dictionary, color: int) -> bool:
	...
	var size: int = board.size
	# 正交：全为该组棋子
	var ortho: Array = board.neighbors(row, col)
	for n in ortho:
		if not group_set.has(n[0] * size + n[1]):
			return false
	# 对角控制：统计非己方数量（对方棋子 + 空点）
	var diags: Array = board.diagonals(row, col)
	var offboard: int = 4 - diags.size()
	var non_friendly: int = 0
	for d in diags:
		var v: int = board.get_at(d[0], d[1])
		if v != color:
			non_friendly += 1  # 对方或空点
	if offboard > 0:
		# 边/角：要求棋盘内对角线全为己方（非己方=0）
		return non_friendly == 0
	else:
		# 中心：最多 1 个非己方对角线
		return non_friendly <= 1
